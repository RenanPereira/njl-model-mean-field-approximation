from su3_3d_cutoff_int_cross_sections.plotting.integrated_cross_section_plots import plot_integrated_cross_section_vs_temperature
from common_utils.cross_section_data import process_to_ylabel_latex


# Common configurations between plots
fig_dpi = 150
fig_x_size = 6
fig_y_size = 6

# Location of the data and plots folder with respect to calculations folder
data_folder = "su3_3d_cutoff_int_cross_sections/data/setA_CP0p318436/"
plots_folder = "su3_3d_cutoff_int_cross_sections/plots/setA_CP0p318436/"

process = "UUUU"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 8.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.08,
    y_annotation=0.88,
    annotation_vert_space=0.06
)

process = "UDUD"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 8.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.88,
    annotation_vert_space=0.06
)


process = "USUS"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 8.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)


process = "SSSS"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 14.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)


process = "UUBarUUBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 20.0),
    x_num_ticks=5,
    y_num_ticks=6,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)

process = "UUBarDDBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 20.0),
    x_num_ticks=5,
    y_num_ticks=6,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)

process = "UUBarSSBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 3.0),
    x_num_ticks=5,
    y_num_ticks=6,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.08,
    y_annotation=0.88,
    annotation_vert_space=0.06
)

process = "UDBarUDBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 32.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)

process = "USBarUSBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 32.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.85,
    annotation_vert_space=0.06
)

process = "SUBarSUBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 20.0),
    x_num_ticks=5,
    y_num_ticks=6,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)

process = "SSBarUUBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 20.0),
    x_num_ticks=5,
    y_num_ticks=6,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)


process = "SSBarSSBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 20.0),
    x_num_ticks=5,
    y_num_ticks=6,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)

process="UBarUBarUBarUBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 32.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.88,
    annotation_vert_space=0.06
)


process="UBarDBarUBarDBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 20.0),
    x_num_ticks=5,
    y_num_ticks=6,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)

process="UBarSBarUBarSBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 8.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)

process="SBarSBarSBarSBar"
plot_integrated_cross_section_vs_temperature(
    fig_dpi,
    fig_x_size,
    fig_y_size,
    data_specs=[
        (
            [
                data_folder + f"IntegratedCrossSection_setA_{process}_COMPLETE_COV_TMin0p040000_TMax0p300000_CPU0p318436.dat",
            ],  
            r"Method I", 
            "black", 
            2, 
            "-"
        ),
    ],
    path_output_plot=plots_folder + f"integrated_cross_section_setA_{process}_CPCEP.png",
    legend_loc="upper right",
    label_int_cross_section=process_to_ylabel_latex(process),
    xlim=(0.040, 0.300),
    ylim=(0.0, 14.0),
    x_num_ticks=5,
    y_num_ticks=5,
    x_formatter="%.3f",
    y_formatter="%.1f",
    annotation_texts=[
        "set A",
        r"$\mu = \mu_{\mathrm{CEP}}$",
    ],
    x_annotation=0.05,
    y_annotation=0.05,
    annotation_vert_space=0.06
)
