...
def plot_kappa_over_sigmae_temp_vs_temp(
    ...
    data_specs:
        ...
        (path_file_kappa, path_file_sigmae, label, color, linewidth, linestyle)
    """
    print("Building plot: kappa over sigmae*temperature versus temperature.")   
    
    print("Using datafiles:")
    for path_file_kappa, path_file_sigmae, _, _, _, _ in data_specs:
        print(path_file_kappa)
        print(path_file_sigmae)
    print()

    datasets: list[tuple[ThermalConductivityData, ElectricalConductivityData, str, str, int, str]] = []
    for path_file_kappa, path_file_sigmae, label, color, linewidth, linestyle in data_specs:
        data_kappa = ThermalConductivityData(path_file_kappa)
        data_sigmae = ElectricalConductivityData(path_file_sigmae)
        datasets.append((data_kappa, data_sigmae, label, color, linewidth, linestyle))

    # Verify that the kappa and sigmae paired data provided have the same temperature grid
    for data_kappa, data_sigmae, label, color, linewidth, linestyle in datasets :
        if not np.array_equal(data_kappa.get_temperature(), data_sigmae.get_temperature()):
            raise ValueError("Temperature grids between paired kappa and sigmae do not match.")
