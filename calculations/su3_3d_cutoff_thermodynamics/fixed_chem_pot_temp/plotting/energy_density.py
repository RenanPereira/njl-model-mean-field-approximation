...
def plot_energy_vs_temperature(
    ...
    """
    This function plots the energy density vs temperature from FixedChemPotTempData.
    ...
    if label_energy_density is None:
        ...
        ax.set_ylabel(label_energy_density, fontsize=labels_fontsize)
...
def plot_energy_euler_eq_vs_temperature(
    ...
    """
    This function plots the energy density, euler equation vs temperature from FixedChemPotTempData.
    ...
    for filepath, label, color, linewidth, linestyle, euler_calculation in data_specs:
        data = FixedChemPotTempData(filepath)
        datasets.append((data, label, color, linewidth, linestyle, euler_calculation))

    # Create a new figure
    fig, ax = plt.subplots(figsize=(fig_x_size, fig_y_size), dpi=fig_dpi)
    
    for data, label, color, linewidth, linestyle, euler_calculation in datasets:
        if (not euler_calculation):
            ax.plot(
                data.get_temperature(),
                data.get_energy_density(),  
                label=label, 
...
def plot_energy_vs_baryon_chem_pot(
    fig_dpi: int,
    fig_x_size: int,
    fig_y_size: int,
    data_specs: list[tuple[str, str, str, int, str]],
    ...
    """
    This function plots the energy density vs baryon chemical potential from FixedChemPotTempData.
    
    data_specs:
        List of tuples defining datasets and plot styles:
        (filepath, label, color, linewidth, linestyle)
    """
    print("Building plot: energy density versus baryon chemical potential.")
    print_unique_filepaths(data_specs)
